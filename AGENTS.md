...
- 产物命名：Windows `x64-windows` / macOS `aarch64-apple`（arm64 原生）+ `x64-apple`（Intel x64 交叉编译）/ Linux `x64-linux`
...
- **补充**：x64 mac 用**交叉编译**（macos-latest arm64 runner 上 `--target x86_64-apple-darwin`），避免 macos-13 Intel runner 排队极慢、阻塞 Release；矩阵以 `matrix.cargo_target` 区分原生/交叉，交叉条目跳过 `cargo test`、产物在 `target/<target>/release/`
...
- 产物命名：Windows `x64-windows` / macOS `aarch64-apple`（macos-latest 为 arm64）+ `x64-apple`（Intel x64 交叉编译）/ Linux `x64-linux`
- 后续补充：用户指出未发布新版本且缺 x64 mac。原计划用 `macos-13`（Intel x64）runner，实测排队极慢（>15 分钟未分配 runner、阻塞 Release），改为**交叉编译**——在 `macos-latest`（arm64）上用 `--target x86_64-apple-darwin` 交叉编译 x64 mac 产物，立即可用；矩阵加 `cargo_target` 字段区分原生/交叉，交叉条目跳过 `cargo test`、产物在 `target/<target>/release/`，成四平台产物互补（Windows x64 / macOS arm64+x64 / Linux x64）
